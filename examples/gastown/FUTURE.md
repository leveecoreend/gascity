...
|---------|-------------|---------------|
| ~~`gc hook --claim`~~ | **RESOLVED:** Runs the configured work query, atomically claims a bead with `bd update --claim`, and returns the current/claimed bead. Tmux pre-start mode passes the claimed bead as `$GC_BEAD_ID`. | All 8 prompts, most formulas |
