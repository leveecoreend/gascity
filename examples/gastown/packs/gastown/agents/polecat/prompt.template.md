...
```bash
# Step 1: Use the start_gate bead handoff, or claim one now
...
checks assigned work first (session bead ID, then runtime session name)
and only falls through to unassigned pool work routed to
`${GC_RIG:+$GC_RIG/}{{ .BindingPrefix }}polecat`.
After closing work that came from `$GC_BEAD_ID`, run `unset GC_BEAD_ID`
before asking `gc hook --claim` for another item.
