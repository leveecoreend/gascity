...
func TestExecCommandRunnerWithEnvReadsAmbientEnvPerCall(t *testing.T) {
	t.Setenv("GC_DYNAMIC_ENV", "before")

	dir := t.TempDir()
	runner := beads.ExecCommandRunnerWithEnv(map[string]string{
		"GC_CITY_PATH": "/city",
	})
	t.Setenv("GC_DYNAMIC_ENV", "after")

	out, err := runner(dir, "sh", "-c", `printf '%s\n%s\n' "$GC_DYNAMIC_ENV" "$GC_CITY_PATH"`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if lines[0] != "after" {
		t.Fatalf("GC_DYNAMIC_ENV = %q, want per-call value %q", lines[0], "after")
	}
	if lines[1] != "/city" {
		t.Fatalf("GC_CITY_PATH = %q, want override %q", lines[1], "/city")
	}
}
