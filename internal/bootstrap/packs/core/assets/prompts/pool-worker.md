...
that bead; run `gc hook --claim` again or drain if no valid work is available.
...
- `bd ready --assignee="$GC_SESSION_NAME"` — inspect pre-assigned ready work
...
2. Claim if unclaimed: `gc hook --claim`
