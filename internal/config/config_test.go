...
func TestEffectiveWorkQueryDefault(t *testing.T) {
	...
	if !strings.Contains(got, "bd ready --include-ephemeral --metadata-field gc.routed_to=mayor --unassigned --json --limit=1") {
...
func TestEffectiveWorkQueryWithDir(t *testing.T) {
	...
	got := a.EffectiveWorkQuery()
	if !strings.Contains(got, "bd ready --include-ephemeral --metadata-field gc.routed_to=hello-world/polecat --unassigned --json --limit=1") {
		t.Errorf("EffectiveWorkQuery() missing tier 3 routed_to: %q", got)
	}
}
...
func TestEffectiveWorkQueryPoolNameOverride(t *testing.T) {
	...
	got := a.EffectiveWorkQuery()
	if !strings.Contains(got, "bd ready --include-ephemeral --metadata-field gc.routed_to=hello-world/dog --unassigned --json --limit=1") {
...
set -eu
case "$*" in
  "ready --include-ephemeral --metadata-field gc.routed_to=gascity/control-dispatcher --unassigned --json --limit=1")
    printf '[]'
    ;;
  "ready --include-ephemeral --metadata-field gc.routed_to=gascity/workflow-control --unassigned --json --limit=1")
