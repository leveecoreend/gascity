...
type Agent struct {
	...
	// Default for multi-instance agents:
	// "bd update {} --set-metadata gc.routed_to=<qualified-name>".
	// Default for singleton agents:
...
// routing command for the agent's session shape.
//
...
//
// Multi-instance templates keep Assignee empty and use gc.routed_to as shared
// pool demand. Singleton templates also set Assignee to the template identity
// so their Tier-2 ready-assigned lookup can claim the bead directly.
func (a *Agent) DefaultSlingQuery() string {
	q := a.QualifiedName()
	if a.SupportsInstanceExpansion() {
		return "bd update {} --set-metadata gc.routed_to=" + q
	}
