...
|---------|----------------|-------|-------------------|--------------|--------------|
| `work_query` via `gc hook` / `gc hook --claim` and controller probes | Agent config | Trusted operator or pack | Agent's canonical city or rig repo | Inherited secrets are stripped; Gas City projects explicit store/session env. `--claim` sets `BEADS_ACTOR` to the session assignee. | Errors are diagnostic only. Avoid placing secrets in command literals. |
...
| `start_gate` | Agent config | Trusted operator or pack | Session work directory | Provider sets `$GC_START_ENV`; only supported env keys are accepted back into the session. Exit 1 declines startup without quarantine. | Provider warnings should avoid secrets. |
| `pre_start` | Agent config | Trusted operator or pack | Session work directory | Provider-specific runtime env; intended for setup before session start after `start_gate` succeeds. | Provider warnings should avoid secrets. |
