...
| `pre_start` | Agent config | Trusted operator or pack | Session work directory | Provider-specific runtime env; intended for setup before session start. Tmux commands receive `$GC_PRE_START_RESULT`; only supported result env keys are accepted back into the session. | Provider warnings should avoid secrets. |
