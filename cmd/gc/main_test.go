...
`
	...
	if !strings.Contains(out, `STATUS=$?`) || !strings.Contains(out, `exit "$STATUS"`) {
		t.Fatalf("graph-worker prompt missing hard-failure handling for gc hook --claim:\n%s", out)
	}
	...
	if !strings.Contains(out, "$GC_BEAD_ID") {
		t.Fatalf("graph-worker prompt missing active bead env:\n%s", out)
	}
	if strings.Contains(out, "if `bd ready --assignee=\"$GC_SESSION_NAME\"` returns") {
		t.Fatalf("graph-worker prompt still describes the old bd-ready polling loop:\n%s", out)
	}
}
