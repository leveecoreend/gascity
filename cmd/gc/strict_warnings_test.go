...
func TestSplitStrictConfigWarnings_LegacyV1SurfaceWarningsAreNonFatal(t *testing.T) {
	fatal, nonFatal := splitStrictConfigWarnings([]string{
		"city.toml: [[agent]] tables are deprecated in v2; use directory-based agents under agents/<name>/. Run `gc doctor` for migration guidance; PackV1 city config is no longer upgraded in place.",
		"city.toml: [packs] is deprecated in v2; use [imports] + packs.lock. Run `gc doctor` for migration guidance; PackV1 city config is no longer upgraded in place.",
		"city.toml: workspace.includes is deprecated in v2; use [imports]. Run `gc doctor` for migration guidance; PackV1 city config is no longer upgraded in place.",
		"city.toml: workspace.default_rig_includes is deprecated in v2; use root pack.toml [defaults.rig.imports.<binding>]. Run `gc doctor` for migration guidance; PackV1 city config is no longer upgraded in place.",
		`city agent "mayor" shadows agent of the same name from import "gs"`,
	})
